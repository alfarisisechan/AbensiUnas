<nav class="navbar navbar-expand-md" style="background-color: #115173;">
        ...
        <div class="modal fade" id="tambahDataModal" tabindex="-1" aria-labelledby="tambahDataModal<?= $row['username'];?>" aria-hidden="true">
                ...
                <tbody>
                    <?php $no = 1; foreach ($data['mahasiswa'] as $row) :?>
                    <tr>
                        <td><?= $no++;?></td>
                        ...
                        type="button" data-bs-toggle="modal" data-bs-target="#ubahDataModal<?= $row['id_mahasiswa'];?>" style="background: linear-gradient(90deg, rgba(6,190,182,1) 50%, rgba(72,177,191,1) 100%); border-color: transparent; color: white;">Ubah</a>
                        </td>
                        <td>
                        <a href="<?= base_url; ?>/admin/hapusmahasiswa/<?= $row['nim_mahasiswa'] ?>" class="btn" type="button"
                                style="background: #A10035; color: white;">Hapus</a>
                        </td>
                    </tr>
                    <div class="modal fade" id="ubahDataModal<?= $row['id_mahasiswa'];?>" tabindex="-1" aria-labelledby="ubahDataModal<?= $row['username'];?>" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <div class="modal-body" id="edit<?= $row['id_mahasiswa'];?>">
                            <form id="formEdit<?= $row['id_mahasiswa'];?>" method="POST">
                            <input name="id" value="<?= $row['id_mahasiswa'];?>" type="hidden" class="form-control" required>
                            <div class="mb-3">
                                <label class="form-label">NIM</label>
                                <input value="<?= $row['nim_mahasiswa'];?>" name="username" type="text" class="form-control" readonly required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Nama Lengkap</label>
                                <input value="<?= $row['nama_mahasiswa'];?>" name="nama" type="text" class="form-control" required>
                            </div>
                                <div class="d-grid gap-2 mt-4">
                                    <button type="submit" class="btn btn-success btn-lg" style="background: linear-gradient(90deg, rgba(6,190,182,1) 50%, rgba(72,177,191,1) 100%); border-color: transparent; color: white;">Simpan</button>
                                </div>
                    ...
                    $("#edit<?= $row['id_mahasiswa'];?>").on("submit", "#formEdit<?= $row['id_mahasiswa'];?>", function() {
                            $.ajax({
