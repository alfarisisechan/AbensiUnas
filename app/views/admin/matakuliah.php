<nav class="navbar navbar-expand-md" style="background-color: #115173;">
        ...
        <div class="modal fade" id="tambahDataModal" tabindex="-1" aria-labelledby="tambahDataModal" aria-hidden="true">
            ...
            <tbody>
                <?php $no=1; foreach ($data['matakuliah'] as $row) :?>
                <tr>
                    <td><?= $no++; ?></td>
                    <td><?= $row['kode_mk'];?></td>
                    <td><?= $row['nama_mk'];?></td>
                    ...
                    type="button" data-bs-toggle="modal" data-bs-target="#ubahDataModal<?= $row['kode_mk'];?>" style="background: linear-gradient(90deg, rgba(6,190,182,1) 50%, rgba(72,177,191,1) 100%); border-color: transparent; color: white;">Ubah</a>
                    </td>
                    <td>
                    <a href="<?= base_url; ?>/admin/hapusMataKuliah/<?= $row['kode_mk'] ?>" class="btn" type="button"
                            style="background: #A10035; color: white;">Hapus</a>
                    </td>
                </tr>
                <div class="modal fade" id="ubahDataModal<?= $row['kode_mk'];?>" tabindex="-1" aria-labelledby="ubahDataModal<?= $row['username'];?>" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-body" id="edit<?= $row['kode_mk'];?>">
                        <form  id="formEdit<?= $row['kode_mk'];?>" method="POST">
                        <div class="mb-3">
                            <label class="form-label">Kode Mata Kuliah</label>
                            <input value="<?= $row['kode_mk'];?>" name="kode_mk" type="number" class="form-control" readonly required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Nama Mata Kuliah</label>
                            <input value="<?= $row['nama_mk'];?>" name="nama_mk" type="text" class="form-control" required>
                        </div>
                            <div class="d-grid gap-2 mt-4">
                                <button type="submit" class="btn btn-success btn-lg" style="background: linear-gradient(90deg, rgba(6,190,182,1) 50%, rgba(72,177,191,1) 100%); border-color: transparent; color: white;">Simpan</button>
                            </div>
                ...
                <script type="text/javascript">
                                $("#edit<?= $row['kode_mk'];?>").on("submit", "#formEdit<?= $row['kode_mk'];?>", function() {
                                        $.ajax({
