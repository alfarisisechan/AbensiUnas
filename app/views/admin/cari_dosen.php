<nav class="navbar navbar-expand-md" style="background-color: #115173;">
        ...
        <div class="modal fade" id="tambahDataModal" tabindex="-1" aria-hidden="true">
            ...
            if(isset($_GET['keyword'])) {
                $result = mysqli_query($db_link, "SELECT * FROM dosen_tbl WHERE nid_dosen = '.$cari.' OR nama_dosen = '.$cari.'");
            } else {
                $result = mysqli_query($db_link, "SELECT * FROM dosen_tbl");
            }
            $array = array();
            while ($data = mysqli_fetch_assoc($result))
            {
                ...
                for ($i = 0; $i < count($datas); $i++) {
                    ...
                    while($array[$j]['nid_dosen'] == $datas[$i] || $array[$j]['nama_dosen'] == $datas[$i]) { ?>
                <tbody>
                    <tr>
                        ...
                        type="button" data-bs-toggle="modal" data-bs-target="#ubahDataModal<?= $array[$j]['id_dosen'];?>" style="background: linear-gradient(90deg, rgba(6,190,182,1) 50%, rgba(72,177,191,1) 100%); border-color: transparent; color: white;">Ubah</a>
                        </td>
                        <td>
                        <a href="<?= base_url; ?>/admin/hapusDosen/<?= $array[$j]['nid_dosen'] ?>" class="btn" type="button"
                                style="background: #A10035; color: white;">Hapus</a>
                        </td>
                    </tr>
                    <div class="modal fade" id="ubahDataModal<?= $array[$j]['id_dosen'];?>" tabindex="-1" aria-labelledby="ubahDataModal<?= $row['username'];?>" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <div class="modal-body" id="edit<?= $array[$j]['id_dosen'];?>">
                            <form id="formEdit<?= $array[$j]['id_dosen'];?>" method="POST">
                            <input name="id" value="<?= $array[$j]['id_dosen'];?>" type="hidden" class="form-control" required>
                            <div class="mb-3">
                                <label class="form-label">NIDN</label>
                                <input value="<?= $array[$j]['nid_dosen'];?>" name="username" type="text" class="form-control" readonly required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Nama Lengkap</label>
                                <input value="<?=$array[$j]['nama_dosen'];?>" name="nama" type="text" class="form-control" required>
                            </div>
                                <div class="d-grid gap-2 mt-4">
                                    <button type="submit" class="btn btn-success btn-lg" style="background: linear-gradient(90deg, rgba(6,190,182,1) 50%, rgba(72,177,191,1) 100%); border-color: transparent; color: white;">Simpan</button>
                                </div>
                    ...
                    $("#edit<?= $array[$j]['id_dosen'];?>").on("submit", "#formEdit<?= $array[$j]['id_dosen'];?>", function() {
                            $.ajax({
