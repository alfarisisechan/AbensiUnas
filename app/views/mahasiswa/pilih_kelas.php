...
<div class="container p-5" style="height: 896px; ">
        <div class="row mt-2">
            ...
            <div class="col-md-4 p-4" style="background-color: #115173; border-radius: 20px;">
                <h1 class="text-center text-white fw-bold mb-4">Pilih Ruang Kelas</h1>
                <img src="<?= base_url; ?>/dist/image/Class.png" alt="Home" class="img-fluid d-inline-block">
                <?php foreach ($data['mahasiswa'] as $row) :?>
                <form id="check" action="<?= base_url; ?>/mahasiswa/ambil_bukti" method="POST">
                    <select name="kode_mk" class="form-select form-select-sm text-center p-3 mb-4 fw-bold"
                        aria-label=".form-select-sm example" required>
                        <option value="">Pilih Kelas</option>
                        <?php foreach ($data['kelas'] as $rows) :?>
                            <option value="<?= $rows['kode_kelas'];?>|<?= $rows['nid_dosen'];?>|<?= $rows['nama_dosen'];?>"><?= $rows['kode_kelas'];?></option>
                        <?php endforeach; ?>
                    </select>
