        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                if(isset($_GET['keyword'])) {
                    $result = mysqli_query($db_link, "SELECT * FROM absensi_tbl WHERE nim_mahasiswa = '.$cari.' AND nama_mahasiswa = '.$cari.' AND kode_mk ='.$kodemk.'");
                } else {
                    $result = mysqli_query($db_link, "SELECT * FROM absensi_tbl");
                }
                $array = array();
                while ($data = mysqli_fetch_assoc($result))
                {
                    ...
                    for ($i = 0; $i < count($datas); $i++) {
                        for ($j = 0; $j < count($array); $j++) {
                            while($array[$j]['nim_mahasiswa'] == $datas[$i] OR $array[$j]['nama_mahasiswa'] == $datas[$i]) { ?>
                <tr>
                    <td><?php echo $array[$j]['nama_mk']; ?></td>
                    <td><?php echo $array[$j]['kode_kelas']; ?></td>
                    <td><?php echo $array[$j]['nim_mahasiswa']; ?></td>
                    <td><?php echo $array[$j]['nama_mahasiswa']; ?></td>
                    <td><?php echo $array[$j]['tanggal']; ?></td>
                    <td><?php echo $array[$j]['jam']; ?></td>
                    <td><?php echo $array[$j]['jarak']; ?></td>
                    <td class="text-center"><img data-bs-toggle="modal" data-bs-target="#BuktiModal<?= $array[$j]['kode_absensi'];?>" height="100" width="auto"
                            src="<?= base_url; ?>/image/bukti_kehadiran/<?php echo $array[$j]['bukti_abse   nsi'];?>"></td>
                    <td><?php echo $array[$j]['status_absensi']; ?></td>
                    <td><button data-bs-toggle="modal"
                            data-bs-target="#ubahStatusModal<?= $array[$j]['kode_absensi'];?>"
                            class="btn btn-outline-light" type="button"
                            style="background: linear-gradient(90deg, rgba(6,190,182,1) 50%, rgba(72,177,191,1) 100%);">Edit</button>
                    </td>
                </tr>
                <div class="modal fade" id="ubahStatusModal<?= $array[$j]['kode_absensi'];?>" tabindex="-1"
                    aria-labelledby="ubahStatusModal<?= $array[$j]['kode_absensi'];?>" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5 fw-bold" id="ubahStatusModal">
                                    <?= $array[$j]['nama_mahasiswa'];?></h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="modal-body" id="edit<?= $array[$j]['kode_absensi'];?>">
                                <form method="POST" id="formEdit<?= $array[$j]['kode_absensi'];?>">
                                    <input name="kode_absensi" type="hidden" value="<?= $array[$j]['kode_absensi'];?>">
                                    <input name="nim" type="hidden" value="<?= $array[$j]['nim_mahasiswa'];?>">
                                    <input name="kode_mk" type="hidden" value="<?= $array[$j]['kode_mk'];?>">
                                    <select name="status"
                                        class="form-select form-select-sm text-center p-3 mb-4 fw-bold"
                                        aria-label=".form-select-sm example" required>
                                        <option value="<?= $array[$j]['kode_absensi'];?>"><?= $array[$j]['status_absensi'];?></option>
                                        <option value="Hadir">Hadir</option>
                                        <option value="Tidak Hadir">Tidak Hadir</option>
                                    </select>
                                    <div class="d-grid gap-2 mt-4">
                                        <button type="submit" class="btn btn-success btn-lg"
                                            style="background: linear-gradient(90deg, rgba(6,190,182,1) 50%, rgba(72,177,191,1) 100%); border-color: transparent; color: white;">Ubah
                                            Kehadiran</button>
                                    </div>
                                </form>
                                <script type="text/javascript">
                                    $("#edit<?= $array[$j]['kode_absensi'];?>").on("submit", "#formEdit<?= $array[$j]['kode_absensi'];?>", function () {
                                        $.ajax({
                ...
                <div class="modal fade" id="BuktiModal<?= $array[$j]['kode_absensi'];?>" tabindex="-1" aria-labelledby="BuktiModal" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered"">
                        <div class="modal-content">
                        <img class="w-100" src="<?= base_url; ?>/image/bukti_kehadiran/<?= $array[$j]['bukti_absensi'];?>">
                        </div>
                    </div>
                </div>
                <?php
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <img src="<?= base_url; ?>/dist/image/Empty.png" alt="Home"
